...
export default function WebsitePreview({ websiteData, websiteId }) {
  return (
    <div className="bg-white text-gray-900 min-h-screen">
      {/* Navigation */}
      <nav className="flex items-center justify-between p-6 border-b">
        <div className="font-bold text-xl">{websiteData?.companyName || 'Your Company'}</div>
        ...
        <div className="flex items-center space-x-4">
          ...
          <button className="px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50">Register</button>
        </div>
      </nav>
      ...
      <section className="grid md:grid-cols-2 gap-12 items-center p-6 max-w-6xl mx-auto">
        <div>
          ...
          <h1 className="text-5xl font-bold mb-6">
            {websiteData?.tagline || 'Your tagline'} <span className="text-blue-600">{websiteData?.companyName || 'Your Company'}.</span>
          </h1>
          <p className="text-xl text-gray-600 mb-8">
            {websiteData?.description || 'Your description goes here.'}
          </p>
          <button className="px-8 py-4 bg-blue-600 text-white rounded-lg text-lg font-semibold hover:bg-blue-700 transition-colors">
            {websiteData?.ctaText || 'Call to Action'}
          </button>
        </div>
        <div className="flex justify-center">
          <div className="w-96 h-96 bg-transparent rounded-xl overflow-hidden">
            ...
            <div 
              ...
              style={{ display: websiteData?.heroImage ? 'none' : 'flex' }}
            >
      ...
      <section className="p-6 max-w-6xl mx-auto mt-12">
        <h2 className="text-3xl font-bold text-center mb-4">Explore jobs by category</h2>
        <p className="text-center text-gray-600 mb-8">
          Get the most exciting jobs from all around the world and grow your career fast with others.
        </p>
      </section>
    </div>
  );
